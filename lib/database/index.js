...
class Manager {
	...
	console.log("Get All rows by category=>",cat);
			var promise = new Promise((resolve, reject) => {
				this.db.all("SELECT _id, title, category, modified FROM markdown WHERE category = '" + cat + "' ORDER BY title", function(err, row) {
					if (err) {
						reject(err)
					} else {
						resolve(row)
