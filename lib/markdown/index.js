			...
			fs.readFile(file, (err, data) => {
				if (err) {
					...
					if (/scss|js/.test(ext)) {
						...
						fs.writeFile(p, this_.content.replace(/{{content}}/, marked(content)))
					} else if (ext === '.md') {
		...
		co(function*() {
			...
			let results = yield stats.map((s) => {
					if (s.dir) {
						...
						if (/(?:css|js|go|md|scss)$/g.test(s.p))
...
let m = new MarkdownMarker('/home/psycho/RESOURCE/归档/temporary/markdown');
